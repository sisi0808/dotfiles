          ...
          case "Property":
            ...
            if (syntaxStash !== false && token !== null && state.type === "Type") {
              var lowPriorityMatching = (
                ...
                state.name === "custom-ident" && token.type === TYPE2.Ident || // https://drafts.csswg.org/css-values-4/#lengths
...
function customDataSetupCommand(context) {
  return async () => {
    if (!import_coc.extensions.all.find((e) => e.id === "coc-html")) {
      import_coc.window.showWarningMessage(`coc-html is not installed`);
      return;
    }
    const htmlConfig = import_coc.workspace.getConfiguration("html");
    const picked = await import_coc.window.showMenuPicker(["Alpine.js", "petite-vue"], "Which customData do you want to use?");
    switch (picked) {
      case -1:
        import_coc.window.showInformationMessage(`It's been cancelled`);
        break;
      case 0:
        htmlConfig.update("customData", [import_path.default.join(context.extensionPath, "customData", "alpinejs", "html.json")]);
        break;
      case 1:
        htmlConfig.update("customData", [import_path.default.join(context.extensionPath, "customData", "petite-vue", "html.json")]);
        break;
      default:
        import_coc.window.showInformationMessage(`It's been cancelled`);
        break;
...
var SelectorCompletionItemProvider = class {
  constructor() {
    this.start = import_coc2.Position.create(0, 0);
  ...
  watchFile(path2, listener) {
    ...
    const watcher = import_coc2.workspace.createFileSystemWatcher(path2);
  ...
  getStyleSheets(uri) {
    return import_coc2.workspace.getConfiguration("html-css-support", uri.toString()).get("styleSheets", []);
  }
  getPath(uri, path2, ext) {
    const folder = import_coc2.workspace.getWorkspaceFolder(uri.toString());
    const name = ext ? (0, import_path2.join)((0, import_path2.dirname)(path2), (0, import_path2.basename)(path2, ext) + ext) : path2;
    return folder ? (0, import_path2.join)((0, import_path2.isAbsolute)(path2) ? folder.uri : (0, import_path2.dirname)(uri.fsPath), name) : (0, import_path2.join)((0, import_path2.dirname)(uri.fsPath), name);
  }
  parseTextToItems(path2, text, items) {
    (0, import_css_tree.walk)((0, import_css_tree.parse)(text), (node) => {
      ...
      switch (node.type) {
        case "ClassSelector":
          kind = import_coc2.CompletionItemKind.Enum;
          break;
        case "IdSelector":
          kind = import_coc2.CompletionItemKind.Value;
          break;
        default:
          return;
      }
  ...
  async fetchLocal(path2) {
    ...
    const items = [];
    try {
      const content = await import_coc2.workspace.readFile(path2);
      this.parseTextToItems((0, import_path2.basename)(path2), content.toString(), items);
    } catch (error) {
    }
  ...
  async fetchRemote(path2) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        this.parseTextToItems((0, import_path2.basename)(path2), text, items);
      }
    } catch (error) {
    }
    this.cache.set(path2, items);
  ...
  async fetch(uri, path2) {
    if (this.isRemote.test(path2)) {
      ...
      const base = (0, import_path2.basename)(uri.fsPath, (0, import_path2.extname)(uri.fsPath));
  ...
  findEmbedded(uri, keys, text) {
    ...
    while ((style = findStyles.exec(text)) !== null) {
      this.parseTextToItems((0, import_path2.basename)(uri.fsPath), style[1], items);
    }
  ...
  async findInherited(uri, keys, text, level = 0) {
    ...
    if (extended && level < 3) {
      ...
      const ext = (0, import_path2.extname)(name) || (0, import_path2.extname)(uri.fsPath);
      const path2 = this.getPath(uri, name, ext);
      const file = import_coc2.Uri.file(path2);
      let text2 = this.files.get(path2);
      if (!text2) {
        try {
          text2 = (await import_coc2.workspace.readFile(file.toString())).toString();
        } catch (error) {
          text2 = "n/a";
        }
  ...
  async findAll(document) {
    ...
    this.findEmbedded(import_coc2.Uri.parse(uri), keys, text);
    await this.findFixed(import_coc2.Uri.parse(uri), keys);
    await this.findLinks(import_coc2.Uri.parse(uri), keys, text);
    await this.findInherited(import_coc2.Uri.parse(uri), keys, text);
    ...
    keys.forEach(
      (key) => {
        ...
        return (_a = this.cache.get(key)) == null ? void 0 : _a.forEach((e) => {
          const res = e.kind === import_coc2.CompletionItemKind.Value ? ids : classes;
    ...
    while ((attribute = findAttribute.exec(text)) !== null) {
      ...
      while ((value = findSelector.exec(attribute[3])) !== null) {
        if (!(attribute[1] === "id" ? context.ids : context.classes).has(value[1])) {
          ...
          diagnostics.push(
            import_coc2.Diagnostic.create(
              import_coc2.Range.create(start, end),
              `CSS selector '${value[1]}' not found.`,
              import_coc2.DiagnosticSeverity.Information
            )
          );
        }
      }
  ...
  provideCompletionItems(document, position) {
    return new Promise((resolve) => {
      const range = import_coc2.Range.create(this.start, position);
      const text = document.getText(range);
      const canComplete = this.canComplete.exec(text);
      if (canComplete) {
        this.findAll(document).then(
          (context) => resolve([...(canComplete[1] === "id" ? context.ids : context.classes).values()])
        );
      } else {
        resolve([]);
      }
...
async function activate(context) {
  ...
  context.subscriptions.push(
    import_coc3.commands.registerCommand("html-css-support.dispose", () => provider.dispose()),
    import_coc3.languages.registerCompletionItemProvider("html-css-support", "HCS", enabledLanguages, provider),
    provider
  );
  context.subscriptions.push(
    import_coc3.commands.registerCommand("html-css-support.customDataSetup", customDataSetupCommand(context))
  );
}
function deactivate() {
}
